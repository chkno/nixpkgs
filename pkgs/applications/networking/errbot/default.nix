...
let
  py = python.override {
    packageOverrides = final: prev: {
      ...
      markdown = prev.markdown.overridePythonAttrs (oldAttrs: rec {
        version = "2.6.11";
        src = prev.fetchPypi {
          pname = "Markdown";
          inherit version;
      ...
      slackclient = prev.slackclient.overridePythonAttrs (oldAttrs: rec {
        version = "1.2.1";
        pname = "slackclient";
        src = fetchFromGitHub {
          owner  = "slackapi";
        ...
        propagatedBuildInputs = with final; [ websocket_client requests six ];

        checkInputs = with final; [ pytest codecov coverage mock pytestcov pytest-mock responses flake8 ];
